  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await updateSearch(id, {
        ...
        // When stopoverPlan is set, legs are auto-generated by the scraper.
        legs: (search.mode ?? 'roundtrip') === 'split' && !search.stopoverPlan
          ? (search.legs ?? [])
          : null,
        stopoverPlan: search.stopoverPlan ?? null,
  ...
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {(search.mode ?? 'roundtrip') === 'split' && (
          <div style={sectionStyle}>
            <h2 style={sectionTitle}>Plan con escala extendida</h2>
            <p style={{ fontSize: 13, color: '#6b7280', margin: '0 0 12px' }}>
              Quedarse 3-4 días en una ciudad intermedia (típicamente Lima) al principio o al final del viaje.
              El sistema genera los tramos automáticamente.
            </p>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, cursor: 'pointer', marginBottom: 12 }}>
              <input
                type="checkbox"
                checked={!!search.stopoverPlan}
                onChange={e => {
                  if (e.target.checked) {
                    update('stopoverPlan', { airport: 'LIM', minDays: 3, maxDays: 4, position: 'end' });
                  } else {
                    update('stopoverPlan', null);
                  }
                }}
              />
              Hacer una parada extendida en una ciudad intermedia
            </label>
            ...
            {search.stopoverPlan && (
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
                <div>
                  ...
                  <input
                    value={search.stopoverPlan.airport ?? ''}
                    onChange={e => update('stopoverPlan', { ...search.stopoverPlan, airport: e.target.value.toUpperCase() })}
                ...
                <div>
                  ...
                  <input
                    type="number" min={1}
                    value={search.stopoverPlan.minDays ?? ''}
                    onChange={e => update('stopoverPlan', { ...search.stopoverPlan, minDays: parseInt(e.target.value, 10) })}
                    style={inputStyle}
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number" min={1}
                    value={search.stopoverPlan.maxDays ?? ''}
                    onChange={e => update('stopoverPlan', { ...search.stopoverPlan, maxDays: parseInt(e.target.value, 10) })}
                    style={inputStyle}
                  />
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={labelStyle}>¿Cuándo hacer la escala?</label>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 4 }}>
                    {[
                      { value: 'start', label: 'Al principio del viaje' },
                      { value: 'end', label: 'Al final del viaje' },
                      { value: 'any', label: 'Cualquiera (busca ambas alternativas)' },
                    ].map(opt => (
                      <label key={opt.value} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
                        <input
                          type="radio"
                          name="stopover-position"
                          value={opt.value}
                          checked={(search.stopoverPlan?.position ?? 'end') === opt.value}
                          onChange={e => update('stopoverPlan', { ...search.stopoverPlan, position: e.target.value })}
                        />
                        {opt.label}
                      </label>
                    ))}
                ...
                <div style={{ gridColumn: '1 / -1', marginTop: 8 }}>
                  <label style={{ display: 'flex', alignItems: 'flex-start', gap: 8, fontSize: 13, cursor: 'pointer' }}>
                    <input
                      type="checkbox"
                      checked={(search.stopoverPlan?.required ?? true)}
                      onChange={e => update('stopoverPlan', { ...search.stopoverPlan, required: e.target.checked })}
                      style={{ marginTop: 2 }}
                    />
                    <span>
                      <strong>Esta parada es obligatoria</strong>
                      <span style={{ display: 'block', color: '#6b7280', fontSize: 12, marginTop: 2 }}>
                        Si la desactivás, el sistema también busca vuelos directos sin la escala
                        y te alerta cuando los encuentre, además de los combos con escala.
                      </span>
                    </span>
                  </label>
                </div>
        ...
        {(search.mode ?? 'roundtrip') === 'split' && !search.stopoverPlan && (
          <div style={sectionStyle}>
